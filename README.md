...
> 기상 피처 11개 중 6개가 상수이고 시정은 고유값이 1개다. 즉 지금 모델은 사실상 기상을
> 쓰지 않으며, 성능은 시간대·노선·항공사에서 나온다. 학습곡선도 570편에서 평평해져
> **표본의 양은 이미 포화**다 — 남은 병목은 양이 아니라 기상 다양성이다.
...
베이스라인도 전체 MAE 14.22분을 받는다. 그러나 15분 이상 지연 구간에서는 MAE 55.26분으로
무너지고 **적중률이 0%** 다 — 중앙값이 음수(-9분)라 한 번도 경고하지 않기 때문이다.
같은 조건에서 사전예측 모델은 전체 MAE 12.86분 · 적중률 17%(오경보율 69%)로 양쪽 모두
베이스라인을 앞선다. 다만 15분 이상 구간의 MAE 는 47.84분으로 여전히 크다.
**정작 예측이 필요한 구간은 두 모델 모두 약하다.**

> 2026-09-16, 1,297편(테스트 260편 · 15분 이상 지연 23편) 기준이다. 표본이 늘면 갱신한다.
