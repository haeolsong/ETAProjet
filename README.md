...
**MAE 만으로 보고하면 결론이 뒤집힌다.** 지연이 0 근처에 몰려 있어 상수 예측인 베이스라인이
전체 MAE 를 낮게 받지만, 15분 이상 지연 구간에서는 MAE 40분대로 무너지고 **적중률이 0%** 다
(한 번도 경고하지 않으므로). 같은 조건에서 사전예측 모델은 44% 를 잡아낸다.
정작 예측이 필요한 구간의 성능은 MAE 순위와 반대라는 뜻이다.
