...
2026-09-13 부터 같은 작업에서 `collect_metar.py` 도 이어서 돈다(`/bin/zsh -c`).
`/bin/sh`(bash) 는 바탕화면 권한 때문에 `getcwd: ... Operation not permitted` 경고를
매 명령마다 로그에 남긴다(수집은 정상). zsh 는 남기지 않는다 — 2026-09-13 launchd 에서 확인.
